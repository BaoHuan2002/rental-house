...
public class EditTenantForm {
   ...
   @FXML
   private Button editTenant_closeBTN;
   ...
   private static Tenant editTenant = null;
   ...
   public static void show(String infrastructureId, Tenant tenant) throws IOException {
      EditTenantForm.infrastructureId = infrastructureId;
      EditTenantForm.editTenant = tenant;
      FXMLLoader loader = new FXMLLoader(EditTenantForm.class.getResource("editTenantForm.fxml"));
      init(loader);
   }
   ...
   public void close() {
      editTenant_closeBTN.getScene().getWindow().hide();
   }

   private void loadTenantData() {
      nameField.setText(editTenant.getName());
      genderComboBox.setValue(new Gender(editTenant.getGender(), editTenant.getGender() == 1 ? "Female" : "Male"));
      phoneField.setText(editTenant.getPhone());
      idNumberField.setText(editTenant.getId_number());
      addressField.setText(editTenant.getAddress());
      img3x4Base64 = editTenant.getImage_3x4();
      frontIdCardBase64 = editTenant.getImage_front_id_card();
      backIdCardBase64 = editTenant.getImage_backside_id_card();

      if (!img3x4Base64.isEmpty()) {
         img3x4.setImage(new Image(new ByteArrayInputStream(Base64.getDecoder().decode(img3x4Base64))));
      }

      if (!frontIdCardBase64.isEmpty()) {
         frontIdCard.setImage(new Image(new ByteArrayInputStream(Base64.getDecoder().decode(frontIdCardBase64))));
      }

      if (!backIdCardBase64.isEmpty()) {
         backIdCard.setImage(new Image(new ByteArrayInputStream(Base64.getDecoder().decode(backIdCardBase64))));
      }
   }
   ...
   private void uploadImg3x4() {
      EditTenantForm.img3x4Base64 = uploadImage("image 3x4", img3x4);
   }

   @FXML
   private void uploadFrontIdCard() {
      EditTenantForm.frontIdCardBase64 = uploadImage("image front id card", frontIdCard);
   }

   @FXML
   private void uploadBackIdCard() {
      EditTenantForm.backIdCardBase64 = uploadImage("image backside id card", backIdCard);
   }
   ...
   private void update() throws IOException {
      ...
      String img3x4 = EditTenantForm.img3x4Base64;
      String frontIdCard = EditTenantForm.frontIdCardBase64;
      String backIdCard = EditTenantForm.backIdCardBase64;
      ...
      } else if ((!ValidateInput.isValidPhone(phone)) || (!phone.matches("^\\d{10}$"))) {
      ...
      } else if ((!ValidateInput.isNumber(idNumber)) || (!idNumber.matches("-?\\d+"))) {
      ...
      Tenant tenant = (editTenant == null) ? new Tenant() : editTenant;
      tenant.setName(name);
      tenant.setGender(genderValue);
      tenant.setPhone(phone);
      tenant.setId_number(idNumber);
      tenant.setAddress(address);
      tenant.setImage_3x4(img3x4);
      tenant.setImage_front_id_card(frontIdCard);
      tenant.setImage_backside_id_card(backIdCard);
      tenant.setInfrastructure_id(EditTenantForm.infrastructureId);
      tenant.setStatus(1);

      boolean success;
      if (editTenant == null) {
         success = tenantController.create(tenant);
      } else {
         success = tenantController.update(tenant);
      }

      if (success) {
         resetAllField();
         DashboardScene.switchToInfrastructureDetail(EditTenantForm.infrastructureId);
         AlertPopup.success("Success", "Tenant updated successfully");
         stage.close();
      }
   }
